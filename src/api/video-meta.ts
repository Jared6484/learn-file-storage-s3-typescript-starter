...
export async function handlerVideoGet(cfg: ApiConfig, req: BunRequest) {
  ...
  //const updatedVideo = dbVideoToSignedVideo(cfg, video);
...
export async function handlerVideosRetrieve(cfg: ApiConfig, req: Request) {
  ...
  //console.log("videos before signing:", videos);
  //const updatedVideos = videos.map(vid => dbVideoToSignedVideo(cfg, vid));
  //console.log("videos after signing:", updatedVideos);

  return respondWithJSON(200, videos); //updatedVideos);
}
